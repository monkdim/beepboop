...
> The listing is generated by CI from the real build, so the API level always matches the
> Dalamud you are running. If the entry looks stale after a game patch, the Release
> workflow just needs a re-run.
